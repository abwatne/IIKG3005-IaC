...
/*
resource "azurerm_subnet" "snet_ase" {
  name = var.snet_ase_name
  resource_group_name  = var.rg_name
  virtual_network_name = azurerm_virtual_network.vnet.name
  ...
  delegation {
    ...
    service_delegation {
      ...
      actions = ["Microsoft.Network/virtualNetworks/subnets/action"] #actions = ["Microsoft.Network/virtualNetworks/subnets/action",]
      }
  }
}*/
